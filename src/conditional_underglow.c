...
#if CU_IS_CENTRAL
    /* Force the throttle to push+render once: _off just queued a strip
     * clear on the lowprio workqueue, and we need our paint to run after
     * it (otherwise the clear leaves the strip dark indefinitely). */
    last_valid = false;
#endif
    cu_request_render();
}
